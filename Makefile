...
sampling-tool: $(objects)
	$(CC) $(PAPI_INCLUDE_FLAGS) $(INSTRO_FLAGS) -g -O3 $(CFLAGS) -o  sampling-tool.so src/stack.c src/driver.c $(LIBMONITOR_FLAGS) $(LDFLAGS)

# Shadow stack ONLY as a library to link against GCC instrumented binaries
libshadowstack-fast:
	$(CC) $(PAPI_INCLUDE_FLAGS) -DSHADOWSTACK_ONLY -O3 	$(CFLAGS) -o libshadowstack-fast.so src/stack.c src/driver.c $(LIBMONITOR_FLAGS) $(LDFLAGS)
libshadowstack-debug:
	$(CC) $(PAPI_INCLUDE_FLAGS) -DSHADOWSTACK_ONLY -DDEBUG -g -O0 $(CFLAGS) -o libshadowstack-debug.so src/stack.c src/driver.c $(LIBMONITOR_FLAGS) $(LDFLAGS)

libemptypushpop:
	$(CC) -O2 -fPIC -shared -o libemptypushpop.so emptypushpop/emptypushpop.c

### Targets & Tests
testStack: libshadowstack-debug
	$(CC) $(PAPI_INCLUDE_FLAGS) -g -I./src -O0 -o test_stack.exe test.c -L. -lshadowstack-debug -L$(LIBMONITOR_BASE)/lib -lmonitor $(LDFLAGS)
